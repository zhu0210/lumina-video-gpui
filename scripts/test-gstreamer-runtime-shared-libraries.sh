...
jq '[.entries[] | {component: "fixture", path: .canonical_path}] | unique_by(.path)' \
    "$inventory" >"$fixture_root/owners"
jq '.[0].component = "wrong-owner"' "$fixture_root/owners" >"$fixture_root/wrong-owner"
! jq -e --argjson expected "$(<"$fixture_root/owners")" \
    'sort_by(.path) == ($expected | sort_by(.path))' "$fixture_root/wrong-owner" >/dev/null
