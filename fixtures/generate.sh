...
rm -rf "$generated_dir"
mkdir -p "$generated_dir/hls-vod" "$generated_dir/hls-event"
...
generate_hls event "$generated_dir/hls-event"

{
    ...
    for playlist in hls-vod/index.m3u8 hls-event/index.m3u8; do
        echo "$playlist:"
        ffprobe -v error -show_entries stream=index,codec_type,codec_name,width,height,channels \
