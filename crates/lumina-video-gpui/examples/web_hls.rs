...
mod web {
    ...
    use lumina_video_gpui::GpuiWebVideoPlayer;

    const DEFAULT_HLS_URL: &str = "https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8";

    thread_local! {
        // The browser event loop outlives the WASM start function.
    ...
    impl Render for WebHlsExample {
        fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
            ...
            div()
                .size_full()
                .font_family("Go Noto Kurrent-Regular")
                .bg(rgb(0x0d1117))
                .child(content)
                .child(
                    div()
        ...
        let handle = gpui_platform::application().run_embedded(|cx: &mut App| {
            if let Err(error) = cx
                ...
                .add_fonts(vec![Cow::Borrowed(include_bytes!(
                    "../../lumina-video-demo/assets/GoNotoKurrent-Regular.ttf"
                ))])
            {
                web_sys::console::error_1(
                    &format!("failed to load video demo font: {error}").into(),
                );
                return;
            }
