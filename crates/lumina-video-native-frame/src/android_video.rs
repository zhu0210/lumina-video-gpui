...
struct SharedState {
    ...
    playback_state: i32,
    /// Matches frame submissions; stale playback-state callbacks cannot relatch EOS.
    generation: u64,
    /// Last error message
...
pub struct AndroidVideoDecoder {
    ...
    started: bool,
    /// Whether the Java Image path can expose producer synchronization (API 33+).
    /// Checked once during initialization; older APIs use CPU-readable output.
    ...
    player_id: u64,
    /// Generation accepted by this decoder; old JNI callbacks cannot refill a seek flush.
    generation: u64,
    /// ExoPlayer's audio clock shared with the existing CorePlayer scheduler.
...
impl AndroidVideoDecoder {
    ...
    pub fn new(url: &str) -> Result<Self, VideoError> {
        ...
        with_jni(&vm, |env| {
            ...
            // Match the Kotlin Image.fence contract, which requires API 33+.
            let ahardwarebuffer_available = {
                let api_level = fetch_android_api_level().unwrap_or(0);
                let available = api_level >= 33;
                if available {
                    info!(
                        "Native Image/HardwareBuffer delivery available (API {}). \
                     The bridge retains Image ownership and completes producer acquire.",
                        api_level
                    );
                } else {
                    info!(
                    "Native Image synchronization unavailable (API {} < 33). Using CPU fallback.",
                    api_level
                );
                }
            ...
            let audio_handle = crate::audio::AudioHandle::default();
    ...
    fn check_state_for_early_return(&self) -> Option<Result<Option<VideoFrame>, VideoError>> {
        ...
        if state.playback_state == STATE_ENDED && !has_pending_hardware_buffer(self.player_id) {
            return Some(Ok(None));
        }
...
impl VideoDecoderBackend for AndroidVideoDecoder {
    ...
    fn decode_next(&mut self) -> Result<Option<VideoFrame>, VideoError> {
        ...
        let frame = PLAYER_QUEUES.get().and_then(|queues| {
            let mut queues = queues.write();
            let state = queues.get_mut(&self.player_id)?;
            if let Some(frame) = state.queue.pop_back() {
                let pts = Duration::from_nanos(u64::try_from(frame.timestamp_ns).ok()?);
                let frame = Arc::new(frame);
                let format = if is_yuv_candidate_hardware_buffer_format(frame.format) {
                    PixelFormat::Nv12
                } else {
                    PixelFormat::Rgba
                };
                // SAFETY: the Arc owns the acquired HardwareBuffer and producer Image.
                // Surface clones keep that exact lease alive through GPU retirement.
                let surface = unsafe {
                    crate::video::AndroidGpuSurface::new(
                        frame.buffer,
                        frame.width,
                        frame.height,
                        format,
                ...
                Some(VideoFrame::new(pts, DecodedFrame::Android(surface)))
            } else {
                state.cpu_frame.take()
            }
        });
    ...
    fn poll_output_transition(&mut self) -> Result<bool, VideoError> {
        ...
        if fallback {
            if let Some(queues) = PLAYER_QUEUES.get() {
                if let Some(state) = queues.write().get_mut(&self.player_id) {
                    state.native_delivery_disabled = true;
                    state.queue.clear();
                    // Keep an already-arrived CPU snapshot: the Kotlin bridge may
                    // have downgraded itself before the renderer requested it.
                }
            }
            let vm = get_jvm()?;
    ...
    fn seek(&mut self, position: Duration) -> Result<(), VideoError> {
        let position_ms = i64::try_from(position.as_millis()).map_err(|_| {
            VideoError::SeekFailed("seek position exceeds Android time range".into())
        })?;
        let generation = self
            .generation
            .checked_add(1)
            .filter(|generation| *generation <= i64::MAX as u64)
            .ok_or_else(|| VideoError::SeekFailed("seek generation exhausted".into()))?;
        let vm = get_jvm()?;
        // Serialize the new generation and mailbox flush with JNI submissions.
        init_player_queues();
        if let Some(queues) = PLAYER_QUEUES.get() {
            let mut queues = queues.write();
            let state = queues
            ...
            state.queue.clear();
            state.cpu_frame = None;
        }
        self.generation = generation;
        {
            let mut state = self.state.lock();
            state.generation = generation;
            state.playback_state = 2; // BUFFERING, never stale ENDED during seek.
        }
        ...
        with_jni(&vm, |env| {
            env.call_method(
                ...
                jni_str!("seek"),
                jni_sig!("(JJ)V"),
                &[JValue::Long(position_ms), JValue::Long(generation as i64)],
            )
    ...
    /// Android ExoPlayer handles audio internally - no separate audio decoder is needed.
    ...
    fn buffering_percent(&self) -> i32 {
        if self.state.lock().playback_state == 2 {
            0
        } else {
            100
        }
...
impl AndroidVideoDecoder {
    ...
    pub fn get_position(&self) -> Result<Duration, VideoError> {
        ...
        with_jni(&vm, |env| {
            ...
            let position_ms = result.j().map_err(|error| {
                VideoError::DecodeFailed(format!("invalid ExoPlayer position: {error}"))
            })?;
            let position_ms = u64::try_from(position_ms).map_err(|_| {
                VideoError::DecodeFailed("ExoPlayer reported a negative position".into())
            })?;
            Ok(Duration::from_millis(position_ms))
        })
    }
...
fn with_player_stats(player_id: u64, f: impl FnOnce(&PerPlayerStats)) {
    ...
    let state = guard.entry(player_id).or_insert_with(PlayerState::new);
    f(&state.stats);
}
...
impl PlayerState {
    fn new() -> Self {
        Self {
            ...
            native_delivery_disabled: false,
            ...
            Ok(frame) => {
                ...
                if let Some(queues) = PLAYER_QUEUES.get() {
                    ...
                    let state = queues
                        ...
                        .or_insert_with(PlayerState::new);
                    if state.generation != generation as u64 {
                        return Ok(true); // Obsolete frame discarded, not an import failure.
                    }
    ...
    if player_id_u64 == 0 {
        ...
        if let Some(queues) = PLAYER_QUEUES.get() {
            ...
            if generation.is_some_and(|generation| {
                generation != state.generation || state.native_delivery_disabled
            }) {
                return; // Dropping the owned frame releases its Image and buffer.
            }
