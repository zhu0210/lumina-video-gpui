...
//!
//! - [`MoqDecoder`]: macOS decoder with VideoToolbox
//! - [`MoqAndroidDecoder`]: Android decoder using MediaCodec with zero-copy HardwareBuffer
//! - Linux feeds encoded MoQ audio/video into `GStreamerDecoder`, which retains
//!   the same native-frame leases and presentation clock as other Linux media.
//! - Unsupported generic decoder entry points fail instead of fabricating frames.
//!
//! # Architecture
//!
...
use super::video::{DecodedFrame, PixelFormat};
use super::video::{HwAccelType, VideoDecoderBackend, VideoError, VideoFrame, VideoMetadata};
#[cfg(target_os = "macos")]
...
impl MoqDecoderConfig {
    ...
    pub(crate) fn apply_localhost_tls_bypass(&mut self, moq_url: &MoqUrl) {
...
impl MoqDecoder {
    ...
    pub fn new_with_config(url: &str, mut config: MoqDecoderConfig) -> Result<Self, VideoError> {
        if !cfg!(target_os = "macos") {
            return Err(VideoError::UnsupportedFormat(
                "MoqDecoder requires macOS VideoToolbox; this platform's native MoQ decoder is not integrated with the player".to_string(),
            ));
        }
    ...
    /// Other platforms must use their native encoded-frame decoder.
    #[cfg(target_os = "macos")]
    ...
    /// Reject unsupported decoding; fabricated pixels must never count as playback.
    #[cfg(not(target_os = "macos"))]
    fn decode_frame(&mut self, _moq_frame: &MoqVideoFrame) -> Result<VideoFrame, VideoError> {
        Err(VideoError::UnsupportedFormat(
            "No encoded-frame decoder is integrated for MoqDecoder on this platform".to_string(),
        ))
    }
...
mod tests {
    ...
    #[test]
    #[cfg(not(target_os = "macos"))]
    fn unsupported_moq_decoder_fails_before_connecting() {
        assert!(matches!(
            MoqDecoder::new("moq://localhost/live/stream"),
