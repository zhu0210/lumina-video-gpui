...
use gpui::prelude::*;
use gpui::{
    div, px, relative, rgb, rgba, size,
    Bounds, FontWeight, WindowBounds, WindowOptions, TitlebarOptions,
    SharedString, App, Window, MouseButton, KeyDownEvent,
};
...
fn main() {
    ...
    let app = gpui::Application::with_platform(std::rc::Rc::new(
        gpui_windows::WindowsPlatform::new(false)
            .expect("Failed to initialise Windows platform"),
    ));
    #[cfg(not(any(target_os = "linux", target_os = "freebsd", target_os = "macos", target_os = "windows")))]
    compile_error!("unsupported platform — lumina-video GPUI demo requires Linux, macOS, or Windows");
...
impl Render for DemoApp {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let play_icon = if is_ended { "↺" } else if is_playing { "⏸" } else { "▶" };
        let is_muted = self.player.as_ref().map_or(false, |p| p.is_muted());
        let mute_icon = if is_muted { "🔇" } else { "🔊" };
        let position = self.player.as_ref().map_or(Duration::ZERO, |p| p.position());
        ...
        div()
            ...
            .on_key_down(cx.listener(
                |this: &mut DemoApp, event: &KeyDownEvent, _w, cx| {
            ...
            .child(
                div()
                    ...
                    .child(
                        div()
                            .px_2().py_1().cursor_pointer().rounded_sm()
                            .hover(|d| d.bg(rgb(0x21262d)))
                            .on_mouse_down(MouseButton::Left, cx.listener(|this, _e, _w, cx| {
                                if this.selected_sample > 0 { this.selected_sample -= 1; cx.notify(); }
                            }))
                            .child("◀"),
                    )
                    .child(div().px_2().py_1().text_sm().font_weight(FontWeight::MEDIUM).child(sel_name.to_string()))
                    // ▶ Next
                    .child(
                        div()
                            .px_2().py_1().cursor_pointer().rounded_sm()
                            .hover(|d| d.bg(rgb(0x21262d)))
                            .on_mouse_down(MouseButton::Left, cx.listener(|this, _e, _w, cx| {
                                if this.selected_sample + 1 < SAMPLE_VIDEOS.len() { this.selected_sample += 1; cx.notify(); }
                            }))
                            .child("▶"),
                    )
                    // URL
                    .child(
                        div()
                            .flex_1().h(px(30.0)).bg(rgb(0x0d1117))
                            .border_1().border_color(rgb(0x30363d)).rounded_sm()
                            .px_2().flex().items_center().text_sm().text_color(rgb(0x8b949e))
                            .overflow_hidden()
                    ...
                    .child(
                        div()
                            .px_4().py(px(6.0)).bg(rgb(0x238636)).rounded_md()
                            .text_sm().font_weight(FontWeight::BOLD).cursor_pointer()
            ...
            .child(
                div()
                    ...
                    .child(
                        div()
                            .flex_1().relative().bg(rgb(0x000000))
                            ...
                            .child({
                                if is_ready {
                                    ...
                                    div()
                                        ...
                                        .child(
                                            div().flex().flex_col().items_center().gap_2()
                                                .child(div().text_xl().text_color(rgb(0xcccccc)).child("⏳"))
                                                .child(div().text_sm().text_color(rgb(0x999999)).child("Loading...")),
                                        )
                                        .into_any_element()
                                } else {
                                    // No player
                                    div()
                                        ...
                                        .child(div().text_color(rgb(0x484f58)).text_lg().child("No video loaded"))
                                        .into_any_element()
                                }
                            })
                            // Buffering overlay
                            .child({
                                if is_playing && buffering < 100 {
                                    div()
                                        .absolute().size_full().bg(rgba(0x00000088))
                                        .flex().flex_col().items_center().justify_center().gap_2()
                                        .child(div().text_lg().text_color(rgb(0x64b4ff)).child(format!("{buffering}%")))
                                        .child(div().text_xs().text_color(rgb(0xcccccc)).child("Buffering..."))
                                        .into_any_element()
                                } else {
                            ...
                            .child(
                                if show_controls {
                                    div()
                                        ...
                                        .h(px(40.0)).bg(rgba(0x000000cc))
                                        .flex().flex_row().items_center().px_2().gap_2()
                                        // Play/pause
                                        .child(
                                            div()
                                                .px_1().cursor_pointer().text_color(rgb(0xffffff))
                                                .hover(|d| d.bg(rgba(0xffffff22))).rounded_sm()
                                                .on_mouse_down(MouseButton::Left, cx.listener(|this, _e, _w, cx| {
                                                    if let Some(ref mut p) = this.player {
                                                        p.toggle_playback();
                                                        cx.notify();
                                        ...
                                        // Time
                                        .child(div().text_sm().text_color(rgb(0xcccccc)).child(pos_text.clone()))
                                        // Seek bar (visual only)
                                        .child(
                                            div().flex_1().h(px(20.0)).flex().items_center()
                                                .child(
                                                    div().w_full().h(px(4.0)).bg(rgba(0xffffff33)).rounded_full()
                                                        .child(
                                                            div().h_full().bg(rgb(0x3b82f6)).rounded_full()
                                                                .w(relative(seek_progress)),
                                                        ),
                                                ),
                                        )
                                        // Duration
                                        .child(div().text_sm().text_color(rgb(0xcccccc)).child(dur_text.clone()))
                                        // Mute
                                        .child(
                                            div()
                                                .px_1().cursor_pointer().text_color(rgb(0xffffff))
                    ...
                    .child(
                        div()
                            .w(px(220.0)).bg(rgb(0x161b22))
                            .border_l_1().border_color(rgb(0x30363d))
                            .flex().flex_col().p_3().gap_1()
                            ...
                            .children({
                                ...
                                SAMPLE_VIDEOS.iter().enumerate().map(move |(i, (name, _))| {
                                    let selected = i == sel;
                                    div()
                                        .px_2().py_1().rounded_sm().text_sm().cursor_pointer()
            ...
            .child(
                div()
                    .h(px(24.0)).flex().items_center().px_2()
                    .bg(rgb(0x161b22)).border_t_1().border_color(rgb(0x30363d))
                    .text_xs().text_color(rgb(0x8b949e))
...
fn section(title: &str) -> impl IntoElement {
    div()
        .font_weight(FontWeight::BOLD).text_sm().mb_2()
...
fn info_row(label: &str, value: &str) -> impl IntoElement {
    div()
        ...
        .child(div().text_xs().text_color(rgb(0xe6edf3)).child(value.to_string()))
}

fn kb(key: &str, desc: &str) -> impl IntoElement {
    div()
        .flex().flex_row().gap_2()
        .child(div().text_xs().text_color(rgb(0x58a6ff)).w(px(48.0)).child(key.to_string()))
        .child(div().text_xs().text_color(rgb(0x8b949e)).child(desc.to_string()))
}
